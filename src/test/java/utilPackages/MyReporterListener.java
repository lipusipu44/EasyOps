...
public class MyReporterListener extends MyReporterListenerAdapter {
	...
	SimpleDateFormat DateFunctions = new SimpleDateFormat("hh:mm:ss");

	static PropertyValExtractors p;

	static String browser=browserPreference();
	...
	public void generateReport(List<XmlSuite> xml, List<ISuite> suites,
		...
		m_out.flush();
		m_out.close();
		L.info("Sending mail with the result");
		SendEmail.eMail();// this method is used to send mail, as this listener is used at the end
		//so have called this method here
	}

	protected PrintWriter createWriter(String outdir) throws IOException {
		java.util.Date now = new Date();
		new File(outdir).mkdirs();
		L.info("Report file is generated");
		return new PrintWriter(new File(System.getProperty("user.dir")+"//Result//current//", "emailable-report.html"));
	}
	...
	protected void generateMethodSummaryReport(List<ISuite> suites) {
		...
		for (ISuite suite : suites) {
			if (suites.size() > 1) {
				titleRow(suite.getName(), 6); //changed from 5 to 6
			}
		...
		if (tests.getAllResults().size() > 0) {
			...
			for (ITestNGMethod method : getMethodSet(tests, suite)) {
				...
				if (mq == 0) {
					...
					titleRow(testname + " &#8212; " + style.toUpperCase() + details, 6, id); //changed from 5 to 6
					m_testIndex = null;
				}
				...
				for (ITestResult testResult : tests.getResults(method)) {
					if (testResult.getEndMillis() > end) {
						...
						endstring=DateFunctions.format(end);//added this to get the time in hh:mm:ss format
					}
					if (testResult.getStartMillis() < start) {
						start =testResult.getStartMillis();
						startstring=DateFunctions.format(start);//added this to get the time in hh:mm:ss format
					}
				}
				mq += 1;
				if (mq > 1) {
				...
				buff.append("<td><a href=\"#m"
						...
						+ (description != null && description.length() > 0 ? "(\""
								...
								+ (null == testInstanceName ? "" : "<br>("
										...
										+ "<td>" + startstring + "</td>"+"<td>" + endstring + "</td>" + "<td class=\"numi\">" //added endstring
										+ (end - start)/1000 + "</td>" + "</tr>"); //changed from millisec to sec
			}
			if (mq > 0) {
				cq += 1;
				m_out.print("<tr class=\"" + style
	...
	private void startResultSummaryTable(String style) {
		...
		m_out.println("<tr><th>Class</th>"
				+ "<th>Method</th><th># of<br/>Scenarios</th><th>Start</th><th>End</th><th>Time<br/>(sec)</th></tr>");//added End here, changed time from ms to sec
		m_row = 0;
	}
	...
	public void generateSuiteSummaryReport(List<ISuite> suites) {
		...
		tableColumnStart("Total<br/>Time (in Sec)");
		...
		for (ISuite suite : suites) {
			...
			for (ISuiteResult r : tests.values()) {
				...
				m_out.print("<td class=\"numi" + (true ? "" : "_attn") + "\">");
				//m_out.println(suite.getParameter("browserType").substring(0, 3));
				m_out.println(browser);
				...
				summaryCell(
						DateFunctions.format(overview.getStartDate()),
						true);
				m_out.println("</td>");
				summaryCell(
						DateFunctions.format(overview.getEndDate()),
						true);
				m_out.println("</td>");
	...
	protected void startHtml(PrintWriter out) {
		...
		out.println("<head>");
		out.println("<title>EasyOps - TestNG Report</title>");
	...
	protected void endHtml(PrintWriter out) {
		...
		try {
			...
			ha = localMachine.getHostAddress();
		} catch (UnknownHostException e) { }
		final String uname= System.getProperty("user.name");
		final String osname = System.getProperty("os.name") + "-"+System.getProperty("sun.os.patch.level");
		final String jname = "Java:"+System.getProperty("java.runtime.version")+"("+System.getProperty("sun.arch.data.model")+")";
		final DateFormat dateFormat = new SimpleDateFormat("HH:mm:ss z 'on' E dd MMM yyyy");

		out.print("<div style=\"text-align: center; margin:0 auto;\">" + "Generated by Anil KUmar Patro at " + dateFormat.format(Calendar.getInstance().getTime()) +
				"<br>" + uname + "("+hn+"/"+ha+") / "+osname + " / "+jname + "</div>");

		out.println("<center> Report customized by Anil Patro [anilpatro044@gmail.com] </center>");
